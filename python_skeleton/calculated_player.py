...
class Player(Bot):
    ...
    def get_action(self, observation: dict) -> Action:
        ...
        # print(len(observation["board_cards"]))
        # print(observation["my_cards"])

        if len(observation["board_cards"]) == 0:
            temp_cards_list = observation["my_cards"]
            temp_cards_list_s = sorted(temp_cards_list, key=lambda x: (int(x[0]), x[1]))
        ...
        if len(observation["board_cards"]) == 1:
            ...
            temp_cards_list_s = sorted(temp_cards_list, key=lambda x: (int(x[0]), x[1]))
            key = temp_cards_list_s[0] + "_" + temp_cards_list_s[1] + "_" + temp_cards_list_s[2]
            equity = self.evalof3[key]
        
        if len(observation["board_cards"]) == 2:
            temp_cards_list = observation["my_cards"] + observation["board_cards"]
            temp_cards_list_s = sorted(temp_cards_list, key=lambda x: (int(x[0]), x[1]))
            key = temp_cards_list_s[0] + "_" + temp_cards_list_s[1] + "_" + temp_cards_list_s[2] + "_" + temp_cards_list_s[3]
        ...
        if equity > my_contribution:
            ...
            if bid_diff > 0.9*expected_diff and CallAction in observation["legal_actions"]:
                action = CallAction()
            if bid_diff <= 0.9*expected_diff and RaiseAction in observation["legal_actions"]:
